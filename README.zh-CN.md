...
普通房间的添加／删除 AI 请求不包含难度值。真正的 AI 行为参数位于比赛开始的 `GrCommandStartPacket` 中，每名 AI 对应 6 个编码 float。当前 Rust 服务器为所有 AI 使用 `[0.7, 2400, 2950, 1.5, 1000, 1500]`。旧 C# 启动器中的“简单／困难／地狱”只是服务器用不同范围生成这 6 个值的预设，并不是客户端上传的原生枚举。

因此以后可以在不修改客户端的情况下增加服务器预设，但这 6 个字段各自的原始名称尚未完全确认。v0.2.6 只在 [AI 难度审计](AI_DIFFICULTY_AUDIT.md)中记录静态边界，不提供带猜测含义的设置。对决任务难度和战斗模式 AI 属于其他路径。
