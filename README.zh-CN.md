...
默认只自动发放名称已解析、具有 `BodyParam`、存在实际 `model.1s`，并且不带测试、dummy 或 NPC 特征的车辆。在原版 P5136 数据中，1,296 辆车里有 1,284 辆通过检查。为避免滚动库存时客户端崩溃，12 个 ID（`199, 312, 323, 352, 657, 658, 659, 744, 745, 746, 814, 886`）保持隔离。若确认某辆隔离车辆可以正常使用，请输入其准确数字 ID，并选择标有“需要人工确认”的结果。该操作只为对应昵称添加序列号 2 或更高的副本，不会重新启用默认序列号 1 的自动发放。
...
默认库存继续保留已经在 P5136 实机确认的 Kartneck／Kartneck X，即使其历史内部名含 `dummyBox`。Boxter HT-S／HT-B／HT LE 的共享模型布局不完整，在原版客户端中表现得更像 dummy 数据，因此重新隔离。另有 4 个缺少韩服／默认 BodyParam 的车体和 5 个明确的测试或占位车体继续排除。
...
继续恢复消费路径后可以确认：前两个值相乘得到 AI 的基础目标速度，第三个值是加速器持续时间（毫秒），第四个值是加速器加速度倍率。第五、六个值会通过 P5136 编解码器，但 `GoBasicAiKart` 不会读取；在此版本中应视为兼容性保留字段。精确公式以及客户端 `basicAI.xml` 中与之互补的参数已记录在 [AI 难度审计](AI_DIFFICULTY_AUDIT.md)中。对决任务难度和战斗模式 AI 属于其他路径。
...
标记不区分大小写，并使用 ASCII 字母数字边界，因此 `TESTS1ROOM` 和 `S10` 不会误匹配。修改房间标题会立即广播标题和密码，但物理预设从下一场比赛开始应用。未填写标记时，普通竞速和道具频道都使用 P5136 原版 `channel.xml` 指定的综合 S7，个人/组队无限加速频道使用普通 S4。道具个人/组队通过 game type 2/4 选择各自的道具物理行，而不是使用不同的 speed byte。S6 是活动专用特殊无限加速预设；S8 在客户端中保留了“标准速度”显示键和物理槽，但没有绑定到启用的原版频道，因此只作为显式手动预设使用。每名玩家会收到一个 235 字节的开赛物理块，由房间默认值与该玩家的车辆、宠物和装备共同组成。
...
连接器中的“显示受限/隐藏特殊赛道（实验性）”只恢复 P5136 同时保留实际赛道 RHO 与普通 `gameType` 定义的项目。当前范围包括 3 条 TF 赛道（汽车人基地、惊破天袭击地球、赛博坦星球之谜）以及 6 条通过静态完整性检查的独立受限赛道。没有普通赛道定义的卡梅洛特、奥林匹斯、韩国、洛奇和冒险岛 `S` 剧情关卡仍会被排除，以避免选择界面卡死或崩溃。
