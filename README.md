...
토큰은 대소문자를 구분하지 않으며 ASCII 영숫자 경계를 따릅니다. 따라서 `TESTS1ROOM`이나 `S10`은 물리 토큰으로 인식하지 않습니다. 방장이 대기실에서 방 제목을 바꾸면 제목과 비밀번호가 모든 참가자에게 즉시 갱신되고, 새 제목의 S0~S8 토큰은 다음 경기 시작 물리에 적용됩니다. 원본 C#과 같이 이미 생성된 방의 채널/세션 speed byte는 제목 변경으로 바뀌지 않습니다. 토큰이 없으면 일반 스피드 채널은 S7, 아이템 채널은 S8, 개인/팀 무한부스터 채널은 S6를 기본값으로 사용합니다. 아이템 개인/팀 채널은 각각 해당 game type의 아이템 물리 행과 S8을 사용합니다. 각 플레이어의 카트·펫·장비와 이 기본값을 합성한 235바이트 경기 시작 물리 블록이 전송됩니다.
...
GUI는 서버·접속기 탭의 입력 설정을 운영체제별 앱 설정 저장소에 기록하고 다음 실행에 복원합니다. 여기에는 IP·포트·클라이언트/프로필 경로·닉네임·실행 방식·Wine/CrossOver/Sikarugir 값·고급 제한값·체크박스·편집한 아이템 확률표·랜덤 트랙 선택이 포함됩니다. 실행 중/정지 상태, 로그 경로, 불러온 카탈로그, 검색 결과처럼 다시 계산해야 하는 일시 데이터는 저장하지 않습니다.
