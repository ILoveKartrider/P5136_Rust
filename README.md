...
토큰은 대소문자를 구분하지 않으며 ASCII 영숫자 경계를 따릅니다. 따라서 `TESTS1ROOM`이나 `S10`은 물리 토큰으로 인식하지 않습니다. 토큰이 없으면 일반 스피드 채널은 S7, 아이템 채널은 S8, 개인/팀 무한부스터 채널은 S6를 기본값으로 사용합니다. 아이템 개인/팀 채널은 각각 해당 game type의 아이템 물리 행과 S8을 사용합니다. 각 플레이어의 카트·펫·장비와 이 기본값을 합성한 235바이트 경기 시작 물리 블록이 전송되며, 방 목록과 시작 세션의 speed byte도 같은 기본값을 알립니다.
