...
서버는 이름, `BodyParam`, 실제 `model.1s`가 모두 해석되고 테스트/더미/NPC 흔적이 없는 카트만 기본 serial 1로 자동 제공합니다. 기본 P5136 데이터에서는 카트 1,296개 중 1,282개가 이 검사를 통과하며, 나머지 14개(ID `199, 312, 323, 352, 657, 658, 659, 744, 745, 746, 795, 814, 886, 1167`)는 인벤토리 스크롤 크래시를 피하기 위해 자동 지급과 이름 검색에서 격리됩니다. 실제 클라이언트에서 지원됨을 직접 확인했다면 검색란에 정확한 숫자 ID를 입력해 `[수동 확인]` 후보를 선택할 수 있습니다. 이 예외는 기본 serial 1을 다시 열지 않고 해당 닉네임에 serial 2 이상 소유분만 추가합니다.
